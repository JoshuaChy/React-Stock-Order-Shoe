...
export default class ModalExampleTopAligned extends Component {
  ...
  render() {
    ...
    onhashchange = () => {
      ...
      if (url[0] === '#order') {
        const item = modalInstance.getStockData(
          dataInstance.displayAllData(),
          url[1]
        );
        ...
        placeorderInstance.placeOrder(
          id,
          header,
          description,
          modalInstance.getTotal(),
          counter
        );
        ordersInstance.addOrderToList(placeorderInstance.getOrder());
        window.localStorage.setItem(
          'orders',
          JSON.stringify(ordersInstance.getOrders())
        );
      }
    };
    if (counter === 1) {
      ...
      button = (
        <Button icon circular floated="left" onClick={this.handleMinusButton}>
          <Icon name="minus" />
        </Button>
      );
    }
          ...
          <Modal.Actions>
            ...
            <Button
              ...
              content={
                ...
                ' to order ' +
                'R' +
                modalInstance.getTotal().toFixed(2)
              }
              secondary
            />
          </Modal.Actions>
